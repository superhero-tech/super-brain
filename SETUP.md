...
The skills in `7-Skills/` are linked into `.claude/skills/`, so Claude Code picks them up as slash commands. `/ingest`, `/query`, `/lint`, `/project-start`, `/project-update` and `/youtube` work the same way.
